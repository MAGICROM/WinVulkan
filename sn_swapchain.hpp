struct snCamera
{
	...
	glm::mat4 LookAtLH(const glm::vec3& to)
	{
		...
		m4_projection = glm::perspectiveLH(fov,
										...
										z_near,
										z_far);
		m4_projection = glm::perspective(glm::pi<float>() * 0.25f, sn_interface.aspectratio, 0.1f, 100.f);
...
void Pipelines_create()
	...
	pm2.rasterizer.frontFace = VK_FRONT_FACE_CLOCKWISE;
...
void sn_Updates(uint32_t imageIndex){     
		...
		sn_interface.mdX = 0;
		sn_interface.mdY = 0;
		glm::mat4 World = camera.LookAtLH(glm::vec3(camera.v3_position.x, camera.v3_position.y, camera.v3_position.z+1.f));
