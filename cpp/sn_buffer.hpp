...
void Set(uint32_t tex_width,uint32_t tex_height)
{
				uint32_t i = bufferCopyRegions.size();
				VkBufferImageCopy bufferCopyRegion = {};
				bufferCopyRegion.bufferOffset = tex_width * tex_height * 4 * i;
				bufferCopyRegion.bufferRowLength = 0;
				bufferCopyRegion.bufferImageHeight = 0;
				bufferCopyRegion.imageSubresource = {VK_IMAGE_ASPECT_COLOR_BIT,0,i,1};
				bufferCopyRegion.imageExtent = {tex_width,tex_height,1};
...
//Pointer set on a shared memory address accessible par le cpu
...
//GPU Cree un buffer sur la carte graphique uniquement
...
//GPU/CPU Cree un buffer sur la carte graphique et visible par le cpu
...
//GPU/CPU Cree un uniform buffer sur la carte graphique et visible par le cpu
...
//GPU/CPU Cree un buffer sur la carte graphique et visible par le cpu pour transfert
...
void CreateTextureFrom(	uint32_t tex_width,uint32_t tex_height,void* data,uint32_t size,snTextureArray* tex_array = nullptr)
{
	...
	if(tex_array)
		layerCount = tex_array->bufferCopyRegions.size();
    ...
    text_img.arrayLayers = layerCount; 
	...
	if(tex_array)
		...
		vkCmdCopyBufferToImage( copyCmd,stage.vkbuffer,
								pTexture->image,
								VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 
								tex_array->bufferCopyRegions.size(), 
								tex_array->bufferCopyRegions.data());
		CommandImgMemBarrier(copyCmd,pTexture->image,VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,&subresourceRange);
		}
	else
		{
	...
	if(tex_array)img_view.viewType = VK_IMAGE_VIEW_TYPE_2D_ARRAY;
    img_view.format = VK_FORMAT_R8G8B8A8_UNORM;
    img_view.subresourceRange = subresourceRange;
    img_view.components = {
    ...
    smp_info.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
...
//GPU Cree une texture de fichier
...
//CPU Cree une texture de fichier
void AddArrayElm(snTextureArray& array, const char* filename,int* tex_width,int* tex_height, int* tex_channels)
{
	unsigned char* img= STBI_ReadImage(filename,tex_width,tex_height,tex_channels);
	if(img)
	{
		...
		array.Set((*tex_width),(*tex_height));
		delete [] img4;
		return;
		}
...
//GPU Cree une texture de fichiers
void CreateTextureArray(snTextureArray &tex_array, snBuffer* buffers,int tex_width,int tex_height, int tex_channels)
{
	uint32_t mem_size = 0;
	uint32_t number = tex_array.bufferCopyRegions.size();
...
//DX Like detruit le buffer
void Release()
	{
		if(pTexture && vkdevicemem)
		{
